...
fifth optional section begins with the keyword "ChiralIDs" and lists the atom
...
allow for smooth restarts. The *vizsteps* option of the
:doc:`fix_modify <fix_modify>` command is supported by this fix to
control the number of timesteps that reacted atoms are highlighted
in :doc:`dump image <dump_image>` output.
